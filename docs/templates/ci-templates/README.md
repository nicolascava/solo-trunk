...
|---|---|---|---|
| Cloud Build (monorepo) | `cloud-build/cloudbuild-sync-customers.yaml` | Terraform trigger — paths filter | GCP Secret Manager |
...
| `MICROSOFT_OAUTH_REFRESH_TOKEN` | Non-interactive refresh token — obtain once locally via `bun packages/scripts/get-ms-refresh-token.ts` |

## Canonical spec

Full routing rules, required `client.json` fields, and secret-management contract:
`docs/templates/canonical-doc-sync.md`
